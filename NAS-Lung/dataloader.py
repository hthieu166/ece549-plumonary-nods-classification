...
class nlstnod(data.Dataset):
    ...
    def __init__(self, npypath, fnamelst, labellst, featlst, train=True,
        ...
        if self.train:
            ...
            #     file = os.path.join(npypath, fentry)
            #     nodule = np.load(file)
            #     self.train_data.append(nodule[:64, :128, :128])
            #     self.train_labels.append(label)
            # self.train_data = np.concatenate(self.train_data)
            # print(len(fnamelst))
            ...
            for label, fentry in zip(labellst, fnamelst):
                ...
                if not isinstance(fentry,str):
                    ...
                    file = os.path.join(npypath, fentry)
                    nodule = np.load(file)
                    nodule = ndimage.interpolation.zoom(nodule, [0.5,  0.25, 0.25])
                    # nodule = np.asarray(nodule[ 16:48, 48:80, 48:80], dtype = np.float32)
                    nodule = np.asarray(nodule, dtype = np.float32)
                    self.test_data.append(nodule)
                    self.test_labels.append(label)
